					...
					<div class="col-12 col-lg-6">
						...
						<?php if( $offer ): ?>
							<div class="why-choose-us__special-offer">
								...
								<p class="why-choose-us__special-offer-text"><?php echo the_field('special_offer_text_before','option'); ?> <?php echo($category_parent->name); ?> <?php echo($taxonomy->name); ?> <?php echo the_field('special_offer_text_after','option'); ?></p>
					...
					<div class="col-6">
						...
						<div id="specpredlojenie" class="contact-form repair-offer__contact-form mfp-hide popup">
							...
							<form class="form repair-offer__form" method="POST">
								...
								<input name="usluga" value="Запишиcь на <?php $offer = get_field('special_offer_check','option'); ?><?php if( $offer ): ?><?php echo mb_strtolower(get_field('special_offer_text_before','option')); ?> <?php echo($category_parent->name); ?> <?php echo($taxonomy->name); ?> <?php echo the_field('special_offer_text_after','option'); ?><?php else:  ?> ремонт для <?php echo($category_parent->name); ?> <?php echo($taxonomy->name); ?><?php endif; ?>" type="hidden">

								<div class="form-group repair-offer__form-group">

									<?php $offer = get_field('special_offer_check','option'); ?>	
									<?php if( $offer ): ?>
										<div class="form-group__title repair-offer__form-group__title">Запишиcь на <?php echo mb_strtolower(get_field('special_offer_text_before','option')); ?> <?php echo($category_parent->name); ?> <?php echo($taxonomy->name); ?> <?php echo the_field('special_offer_text_after','option'); ?></div>
										<?php else:  ?>
											<div class="form-group__title repair-offer__form-group__title">Запишиcь на ремонт для <?php echo($category_parent->name); ?> <?php echo($taxonomy->name); ?></div>
										<?php endif; ?>
